...
- DXFのHATCH塗り（ソリッド/パターン）自体は非対応。境界の輪郭線のみ描画する
  （下記「HATCH境界（対応済み）」参照）。境界エッジが楕円弧・スプラインの場合も
  未対応（該当HATCHのみスキップ）

### HATCH境界（DXF、輪郭線のみ対応済み）

依存ライブラリ`dxf-parser`(v1.1.2)はHATCHエンティティを一切パースしない
（ソースコードに"hatch"の文字列自体が存在しないことを実測確認済み）。
`parser.registerEntityHandler`（dxf-parserが公開するプラグイン機構）で
自前ハンドラ(`HatchRawHandler`)を登録し、HATCHの生group code列をそのまま
保持させ、`parseHatchLoops`で境界パスデータ（91=パス数、92=パス種別フラグ、
93=エッジ数、72=エッジ種別、line/arcエッジ固有のコード）を解釈する。
塗り（ソリッド/パターン）は描画せず、境界の輪郭だけを他のポリラインと同じ
描画経路（bulge付きpolyline）に載せる。楕円弧・スプラインの境界エッジは
未対応（該当HATCHのみスキップ、ファイル全体には影響しない）。

**罠（実測で発覚）**: 円弧境界エッジのccw(反時計回り)フラグが偽（時計回り）の
場合、単純なstart/end角度の入れ替えや掃引方向の反転だけでは説明が付かない
不整合が生じる。ezdxfドキュメントの確認と、実ファイル（隣接エッジとの
連続性＝前エッジの終点と次エッジの始点が一致するはず、という制約から
逆算）により、格納されているgroup 50/51の角度値自体がX軸に関する鏡映
（`360-角度`の符号反転）を経た値になっていることを特定した
（`testdata/clockwise_arcs_hatch.dxf`、CW/CCW混在の31エッジ・6エッジの
両ループで、全エッジ間のギャップが0になることを数値検証済み）。

**罠2（Claudeレビュー指摘・実データで確認）**: 境界パスがエッジ1本だけの
全周円（`93=1, 72=2, 50=0, 51=360`、穴やアイランドの境界としてAutoCADが
よく生成する形）は、角度差をmod 360で正規化すると差が0になり1点に潰れて
消える（無描画になるだけでエラーにはならないため気付きにくい）。全周円を
検出したら180度ずつ2点＋bulge(±1)に分割することで対処した。
